...
overview: 实现首页卡皮巴拉的智能场景文案、点击交互动效（震动+弹跳）以及基于 Canvas 的眨眼微动效，提升 App 的情绪价值。
todos:
...
为了让首页的卡皮巴拉更加生动有趣，我们将从**智能文案**、**点击互动**和**微动效**三个维度对其进行升级。以下是具体的开发方案：
...
为卡皮巴拉增加“解压玩具”的属性：
*   **震动与视觉反馈**：在 `GreetingMascotView` 为卡皮巴拉添加 `onTapGesture`。点击时触发 `UIImpactFeedbackGenerator(style: .soft)` 提供轻微物理震动，同时使用 `.spring` 动画让 `CapybaraView` 的 `scaleEffect` 瞬间变小（被按扁）再弹回。
